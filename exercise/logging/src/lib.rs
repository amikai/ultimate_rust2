...
impl Frog {
    ...
    pub fn hop(&mut self) {
        ...
        info!(target: "Frog::hop", "{} energy after hop", self.energy);
        if self.energy == 0 {
            // 4. Use warn!() to warn that the frog will go to sleep since he ran out of energy
            warn!(target: "Frog::hop", "energy is run out, sleep");
            self.sleep();
        }
    }
    pub fn sleep(&mut self) {
        if self.sleeping {
            ...
            error!(target: "Frog::sleep", "frog is already sleep");
        } else {
            self.sleeping = true;
        }
    }
...
impl Default for Frog {
    fn default() -> Self {
        ...
        trace!(target: "Frog::default", "default value of Frog was generated: {:#?}", frog);
        frog
    }
}
